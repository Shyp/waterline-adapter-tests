...
describe('Semantic Interface', function() {
  ...
  describe('.update()', function() {
    ...
    before(function(done) {
      ...
      Semantic.User1.destroy(function(err) {
        done(err);
      });
    });
    ...
    describe('attributes', function() {
      ...
      before(function(done) {
        ...
        Semantic.User1.createEach(users, function(err, users) {
      ...
      it('should update model attributes', function(done) {
        Semantic.User1.update({ type: 'update' }, { last_name: 'updated' }, function(err, users) {
          assert.ifError(err);
      ...
      it('should return model instances', function(done) {
       Semantic.User1.update({ type: 'update' }, { last_name: 'updated again' }).exec(function(err, users) {
          assert.ifError(err);
          assert(users[0].id);
      ...
      it('should work with just an ID passed in', function(done) {
        Semantic.User1.update(id, { first_name: 'foo' }).sort('first_name').exec(function(err, users) {
          assert.ifError(err);
      ...
      it('should work with an empty object', function(done) {
        Semantic.User1.update({}, { type: 'update all' }, function(err, users) {
          assert.ifError(err);
          assert.strictEqual(users.length, 10);
      ...
      it('should work with null values', function(done) {
        Semantic.User1.update(id, { age: null }, function(err, users) {
          assert.ifError(err);
    ...
    describe('find updated records', function() {
      ...
      before(function(done) {
        ...
        Semantic.User1.createEach(users, function(err, users) {
          ...
          // Update the 2 users
          Semantic.User1.update({ type: 'updateFind' }, { last_name: 'Updated Find' }, function(err) {
            if(err) return done(err);
            done();
          });
        });
      ...
      it('should allow the record to be found', function(done) {
        Semantic.User1.find({ type: 'updateFind' }, function(err, users) {
          assert.ifError(err);
