...
describe('Semantic Interface', function() {
  ...
  describe('.update()', function() {
    ...
    before(function(done) {
      ...
      Semantic.Usertable.destroy(function(err) {
    ...
    describe('attributes', function() {
      ...
      before(function(done) {
        ...
        Semantic.Usertable.createEach(users, function(err, users) {
          ...
          Semantic.Thingtable.create({ name: 'The Thing', description: 'A thing', age: 10 }, function(err, thing) {
            if(err) return done(err);
            thingId = thing.id;
            done();
          });
      ...
      it('should update model attributes', function(done) {
        Semantic.Usertable.update({ type: 'update' }, { last_name: 'updated' }, function(err, users) {
          assert.ifError(err);
      ...
      it('should return model instances', function(done) {
       Semantic.Usertable.update({ type: 'update' }, { last_name: 'updated again' }).exec(function(err, users) {
          assert.ifError(err);
          assert(users[0].id);
      ...
      it('should work with just an ID passed in', function(done) {
        Semantic.Usertable.update(id, { first_name: 'foo' }).sort('first_name').exec(function(err, users) {
          assert.ifError(err);
      ...
      it('should work with an empty object', function(done) {
        Semantic.Usertable.update({}, { type: 'update all' }, function(err, users) {
          assert.ifError(err);
          assert.strictEqual(users.length, 10);
      ...
      it('should work with null values', function(done) {
        Semantic.Usertable.update(id, { age: null }, function(err, users) {
          assert.ifError(err);
      ...
      it('should update model attributes without supplying required fields', function(done) {
        Semantic.Thingtable.update(thingId, { description: 'An updated thing' }, function(err, things) {
          assert.ifError(err);
    ...
    describe('find updated records', function() {
      ...
      before(function(done) {
        ...
        Semantic.Usertable.createEach(users, function(err, users) {
          ...
          // Update the 2 users
          Semantic.Usertable.update({ type: 'updateFind' }, { last_name: 'Updated Find' }, function(err) {
            if(err) return done(err);
            done();
          });
        });
      ...
      it('should allow the record to be found', function(done) {
        Semantic.Usertable.find({ type: 'updateFind' }, function(err, users) {
          assert.ifError(err);
