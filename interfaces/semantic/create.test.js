...
describe('Semantic Interface', function() {
  ...
  describe('.create()', function() {
    ...
    it('should create a new record', function(done) {
      Semantic.User1.create({ first_name: 'Foo' }, function(err, record) {
    ...
    it('should return a generated PK', function(done) {
      Semantic.User1.create({ first_name: 'FooBar' }, function(err, user) {
        if (err) { console.error(err); }
        assert.ifError(err);
    ...
    it('should return generated timestamps', function(done) {
      Semantic.User1.create({ first_name: 'Foo', last_name: 'Bar' }, function(err, user) {
        if (err) { console.error(err); }
        assert.ifError(err);
    ...
    it('should normalize undefined values to null', function(done) {
      Semantic.User1.create({ first_name: 'Yezy', last_name: undefined }, function(err, user) {
        assert.ifError(err);
    ...
    it('should return rows in the correct order when creating multiple rows', function(done) {
      ...
      Semantic.User1.create(users, function(err, users) {
        assert.ifError(err);
    ...
    describe('overloaded usage of create', function() {
      ...
      before(function(done) {
        ...
        Semantic.User1.create(users, done);
      });
      ...
      it('should have saved the proper values (with auto-increment values)', function(done) {
        Semantic.User1.find({where : { type: testName }, sort : {first_name : 1}}, function(err, users) {
