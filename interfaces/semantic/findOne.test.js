...
describe('Semantic Interface', function() {
  ...
  describe('.findOne()', function() {
    ...
    before(function(done) {
      Semantic.User1.create({ first_name: 'findOne test'}, function(err, record) {
    ...
    it('should return a single record', function(done) {
      Semantic.User1.findOne({ first_name: 'findOne test' }, function(err, user) {
        assert.ifError(err);
    ...
    it('should return null if a record is not found', function(done) {
      Semantic.User1.findOne({ first_name: 'findOne blah' }, function(err, user) {
        assert.ifError(err);
        assert(!user);
        done();
      });
    ...
    it('should work with just an id passed in', function(done) {
      Semantic.User1.findOne(id, function(err, user) {
        assert.ifError(err);
        assert.equal(user.first_name, 'findOne test');
        done();
      });
